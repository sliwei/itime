...
export default function Index() {
  ...
  return (
    <>
      <motion.div {...maskAnimate} className="w-screen h-screen fixed top-0 left-0 z-30 backdrop-blur-xl"></motion.div>
      <motion.div {...fullAnimate} className="rounded-[10px] fixed z-40 top-0 w-full left-0 h-full flex flex-col text-[30px]">
        <div className="flex-1 bg-white rounded-[10px] m-[10px] relative border-[#fae3d9] border-[2px] border-solid">
          ...
          <img onClick={() => setOpen(-1)} className="h-[56px] w-[56px] anim_btn absolute z-20 right-[5px] top-[5px]" src={closeImg} alt="" />
          <div className="absolute z-10 w-full h-full flex flex-col p-[20px] box-border">
            ...
            <div className="mb-[20px] flex gap-[10px] flex-wrap flex-1 content-start">
              {imgList.map((media, index) => (
                <div key={index} className="w-[210px] h-[140px] border-[2px] border-[#fae3d9] border-solid rounded-[10px] box-border overflow-hidden mr-[4px] mb-[4px] relative">
              ...
              >
                ...
                <input onChange={fileChangeFetch} type="file" ref={fileInput} accept="image/*" hidden />
              </div>
            </div>
            ...
            <div className="flex items-center w-full rounded-[10px] border-[2px] box-border border-solid border-[#fff] mb-[20px] p-[5px] text-[30px] bg-[#fae3d9] anim_btn" onClick={setCheckAddrState}>
              <img className="w-[40px] h-[40px] mx-[5px]" src={localImg} />
              地址：{address}
            </div>
            <div className="flex items-center w-full rounded-[10px] border-[2px] box-border border-solid border-[#fff] p-[5px] text-[30px] bg-[#fae3d9] anim_btn" onClick={setCheckDateState}>
        ...
        <div className="w-full h-[140px] flex justify-evenly items-center">
          <div className="anim_btn text-[40px] mt-[-40px] leading-none" onClick={reset}>
            重置
          </div>
          <div className="anim_btn text-[40px] mt-[-40px] leading-none" onClick={saveFetch}>
            保存
          </div>
        </div>
      </motion.div>
      <Modal show={checkAddrState} mode={1}>
        <div className="flex justify-end items-center flex-col h-screen w-screen">
          <div className="absolute z-0 w-full h-full" onClick={setCheckAddrState}></div>
          <div className="absolute z-10 bg-white w-full flex justify-end items-center flex-col py-[40px]">
            ...
            <div className="w-full flex justify-evenly items-center mt-[20px]">
              <div className="anim_btn text-[40px] mb-[20px] leading-none" onClick={setCheckAddrState}>
                取消
              </div>
              <div className="anim_btn text-[40px] mb-[20px] leading-none" onClick={setCheckAddrState}>
                确定
      ...
      <Modal show={checkDateState} mode={1}>
        <div className="flex justify-end items-center flex-col h-screen w-screen">
          <div className="absolute z-0 w-full h-full" onClick={setCheckDateState}></div>
          <div className="absolute z-10 bg-white w-full flex justify-end items-center flex-col py-[40px]">
            <div className="mb-[20px]">选择时间</div>
            <PickerPanel<Dayjs> {...sharedProps} locale={zhCN} showTime />
            <div className="w-full flex justify-evenly items-center mt-[20px]">
              <div className="anim_btn text-[40px] mb-[20px] leading-none" onClick={setCheckDateState}>
                取消
              </div>
              <div className="anim_btn text-[40px] mb-[20px] leading-none" onClick={setCheckDateState}>
                确定
      ...
      <MaskLoading state={saveState.loading || fileChangeState.loading} time={2000} />
    </>
  )
}
